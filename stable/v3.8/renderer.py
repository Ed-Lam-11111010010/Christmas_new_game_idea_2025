...
class GameRenderer:
    ...
    def draw_menu(self, game):
        ...
        t = self.font_title.render(f"CITY ROGUE v3.11", True, WHITE)
        ...
        # Leaderboard (Right Side with Frame)
        lb_x = 400
        lb_w = w - 450
        lb_h = 400
        
        # Frame
        pygame.draw.rect(self.screen, (20, 20, 25), (lb_x, 50, lb_w, lb_h)) # Bg
        pygame.draw.rect(self.screen, GOLD, (lb_x, 50, lb_w, lb_h), 2) # Border
        
        header = self.font_title.render("TOP MAYORS", True, GOLD)
        self.screen.blit(header, (lb_x + 20, 70))
        
        for i, s in enumerate(game.high_scores):
            txt = self.font.render(f"{i+1}. {s['score']} - {s['status']} ({s['date']})", True, WHITE)
            self.screen.blit(txt, (lb_x + 30, 130 + i*40))
    ...
    def draw_game(self, game):
        ...
        if game.popup_queue:
            ...
            ref = int(game.get_cost(game.grid[pr][pc])*0.5)
    ...
    def draw_sidebar(self, game):
        ...
        visible_logs = game.logs[game.log_scroll_offset : game.log_scroll_offset + game.max_log_lines]
